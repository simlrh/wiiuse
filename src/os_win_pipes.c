...
int wiiuse_os_find(struct wiimote_t** wm, int max_wiimotes, int timeout) {
	...
	for (i = 0; i < max_wiimotes; i++) {
		sprintf_s(buffer, 256, "\\\\.\\pipe\\wiimote%d", i);
		dev = CreateFile(buffer,
		...
		wm[i]->dev_handle = dev;
		wm[i]->hid_overlap.hEvent = CreateEvent(NULL, 1, 1, "");
		wm[i]->hid_overlap.Offset = 0;
		wm[i]->hid_overlap.OffsetHigh = 0;

		WIIMOTE_ENABLE_STATE(wm[i], WIIMOTE_STATE_DEV_FOUND);
		WIIMOTE_ENABLE_STATE(wm[i], WIIMOTE_STATE_CONNECTED);

		/* try to set the output report to see if the device is actually connected */
		if (!wiiuse_set_report_type(wm[i])) {
			WIIMOTE_DISABLE_STATE(wm[i], WIIMOTE_STATE_CONNECTED);
			continue;
		}
		...
		WIIUSE_INFO("Connected to wiimote [id %i].", wm[i]->unid);
	}
...
int wiiuse_os_write(struct wiimote_t* wm, byte report_type, byte* buf, int len) {
	...
	return WriteFile(wm->dev_handle, write_buffer, len + 1, &bytes, &wm->hid_overlap);
}
